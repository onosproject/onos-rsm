...
var log = logging.GetLogger("store", "ue-store")
...
type Store interface {
	...
	UpdateUEInfo(ctx context.Context, key string, value interface{}) (*Entry, error)

	UpdateUESlice(ctx context.Context, key string, value interface{}) (*Entry, error)
...
type store struct {
	...
	// key is UUID but will changed to global UE ID
...
func (s *store) UpdateUEInfo(ctx context.Context, key string, value interface{}) (*Entry, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *store) UpdateUESlice(ctx context.Context, key string, value interface{}) (*Entry, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
